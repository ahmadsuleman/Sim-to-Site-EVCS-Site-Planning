#!/usr/bin/env python3
"""
Distribution-aware final EVCS placement v2.

This script creates paper-ready final placement outputs:
  - final ranked candidate table
  - final selected sites table
  - city placement map with road graph and optional OSM basemap
  - placement summary diagnostics

Main fixes included:
  1. Merges coordinates from candidate_sites*.csv when ranking file lacks lat/lon.
  2. Uses full city extent from candidates + demand + graph nodes.
  3. Enforces minimum pairwise separation.
  4. Enforces zone diversity and max-per-zone selection.
  5. Draws the simulator road graph on the final city map.
  6. Optionally adds an OSM basemap if contextily is installed.

Usage:
    python final_placement_v2.py \
      --dataset-index dataset_index.csv \
      --rankings outputs_sim2real/rankings/ranked_candidates_all_methods.csv \
      --output-dir outputs_final_placement_v2 \
...
def find_file(data_dir: Path, patterns: Iterable[str]) -> Optional[Path]:
    """Find the first file in data_dir matching any regex pattern."""
...
def load_optional_csv(data_dir: Path, patterns: Iterable[str]) -> Optional[pd.DataFrame]:
    """Load the first matching CSV file, or None if not found."""
...
def detect_id_col(df: pd.DataFrame, preferred: str = "candidate_id") -> Optional[str]:
    """Detect candidate/site identifier column."""
    if preferred in df.columns:
        return preferred

    candidates = ["site_id", "candidate_site_id", "id", "node_id", "osmid"]
    for c in candidates:
        if c in df.columns:
            return c
    ...
    for c in df.columns:
        cl = c.lower()
        if "candidate" in cl and "id" in cl:
            return c
        if "site" in cl and "id" in cl:
            return c
...
def detect_node_id_col(df: pd.DataFrame) -> Optional[str]:
    """Detect graph node identifier column."""
...
def detect_edge_endpoint_cols(edges: pd.DataFrame) -> Optional[Tuple[str, str]]:
    """Detect source/target endpoint columns in graph edge file."""
    endpoint_pairs = [
        ("u", "v"),
        ("source", "target"),
        ("from", "to"),
...
def load_candidate_sites_with_coords(data_dir: Path) -> pd.DataFrame:
    """Load candidate_sites*.csv and standardize candidate_id, lat, lon."""
...
def ensure_rankings_have_coordinates(rankings: pd.DataFrame, data_dir: Path) -> pd.DataFrame:
    """
    Ranking tables often lack lat/lon. Merge coordinates from candidate_sites*.csv.
    """
    df = rankings.copy()
    ...
    if "candidate_id" not in df.columns:
        raise ValueError(
            "Ranking table has no lat/lon and no candidate_id. "
            "Cannot merge coordinates from candidate_sites*.csv."
        )
    ...
    if missing:
        ...
        raise ValueError(
            ...
            f"Check whether candidate IDs match between ranking file and candidate_sites.csv."
        )

    return merged


def load_city_extent(data_dir: Path, candidate_df: pd.DataFrame) -> pd.DataFrame:
    """Use candidates + demand + graph nodes to determine full city map extent."""
    pieces = [candidate_df[["lat", "lon"]].copy()]

    demand = load_optional_csv(data_dir, [r"demand.*\.csv$"])
    if demand is not None and len(demand):
        try:
            dlat, dlon = detect_lat_lon(demand)
            pieces.append(demand[[dlat, dlon]].rename(columns={dlat: "lat", dlon: "lon"}))
        except Exception:
            pass
...
def assign_grid_zones(df: pd.DataFrame, city_coords: pd.DataFrame, rows: int, cols: int) -> pd.DataFrame:
    """Assign candidates to grid zones based on full city extent."""
    out = df.copy()
...
def add_local_demand_score(candidates: pd.DataFrame, data_dir: Path, radius_km: float = 2.0) -> pd.Series:
    ...
    if demand_mass_col is None:
        demand["_demand_mass"] = 1.0
        demand_mass_col = "_demand_mass"

    d = demand[[dlat, dlon, demand_mass_col]].dropna()
    ...
    for _, c in candidates.iterrows():
        ...
        for _, q in d.iterrows():
            dist = haversine_km(c["lat"], c["lon"], q[dlat], q[dlon])
            if dist <= radius_km:
                total += float(q[demand_mass_col]) * (1 - dist / radius_km)
        scores.append(total)
...
def load_road_graph_for_plot(data_dir: Path, max_edges: int = 15000) -> Optional[pd.DataFrame]:
    """
    Load graph road segments as lon1, lat1, lon2, lat2 for plotting.
    """
    nodes = load_optional_csv(data_dir, [r"graph_nodes.*\.csv$", r"^nodes.*\.csv$"])
...
def apply_map_extent(ax, coords: pd.DataFrame, padding_fraction: float = 0.04) -> None:
    """Set map bounds using city coordinates."""
...
def add_optional_basemap(ax, cfg: dict) -> None:
    """
    Add OSM basemap if contextily is installed and internet/cache is available.
    If not, silently fall back to simulator road graph.
    """
...
def prepare_candidate_pool(
    ...
    """Prepare final candidate pool for one city."""
    ...
    city_rankings["final_rank_score_v2"] = (
        w.get("rank_score", 0.45) * city_rankings["rank_score_norm"] +
        ...
        w.get("underserved_or_equity", 0.05) * city_rankings["equity_norm"]
    )

    city_coords = load_city_extent(data_dir, city_rankings)
...
def can_add_candidate(row: pd.Series, selected: List[pd.Series], min_sep_km: float) -> bool:
    """Check whether row is far enough from already selected candidates."""
    for s in selected:
...
def select_distribution_aware(candidates: pd.DataFrame, cfg: dict) -> Tuple[pd.DataFrame, dict]:
    """Select final EVCS sites using score + spatial distribution rules."""
    fp_cfg = cfg["final_placement"]
    ...
    relaxations = [
        float(x)
        for x in fp_cfg.get("min_separation_relaxation", [fp_cfg.get("min_separation_km", 1.5)])
    ]
...
def pairwise_selected_stats(selected: pd.DataFrame) -> dict:
    """Compute pairwise selected-site distance diagnostics."""
    if len(selected) < 2:
        return {
...
def plot_city_placement(
    ...
    """
    Publication-quality final placement map.

    Layers:
      1. Optional OSM basemap
      2. Simulator road graph
      3. All candidate sites
      4. Top ranked candidates
      5. Final selected EVCS sites
    """
    ...
    ax.set_title(f"Distribution-aware final EVCS placement: {city_id}", fontsize=13)
    ...
    for fmt in plot_cfg.get("figure_format", ["png"]):
        fig.savefig(
            fig_dir / f"final_placement_v2_{city_id}.{fmt}",
            dpi=dpi,
            bbox_inches="tight",
        )
...
def main() -> None:
    ...
    for _, row in real_rows.iterrows():
        ...
        try:
            ...
            candidates_out.to_csv(output_dir / "rankings" / f"final_ranked_sites_v2_{city_id}.csv", index=False)
            selected.to_csv(output_dir / "selected" / f"final_selected_sites_v2_{city_id}.csv", index=False)
            ...
            min_pair = meta.get("min_pairwise_distance_km", np.nan)
            print(
                f"  selected={meta['selected_count']} "
                f"active_zones={meta['active_zones']} "
                f"min_sep_used={meta['min_separation_used_km']} km "
                f"min_pairwise={min_pair:.3f} km"
            )
    ...
    summary_df.to_csv(output_dir / "tables" / "final_placement_v2_summary.csv", index=False)
